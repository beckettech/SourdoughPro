...
enum MockStarters {
    ...
    /// Secondary starter that needs attention.
    static let gerty: Starter = {
        let now = Date()
        let created = Calendar.current.date(byAdding: .day, value: -45, to: now) ?? now
        let feedings = makeFeedings(starterId: gertyId, count: 12, spacingHours: 24, endingAt: now.addingTimeInterval(-48 * 3600))
        var s = Starter(
            id: gertyId,
            name: "Gerty",
            createdAt: created,
            ...
            nextFeedingAt: nil,
            isActive: true,
            notes: "Lives in the fridge, weekly feed schedule."
        )
        s.nextFeedingAt = FeedingScheduler.nextFeeding(for: s)
        return s
    }()
